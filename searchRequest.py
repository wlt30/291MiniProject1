...
def messageRequestOwner(database, index, displayedRequests, member):
    ...
    os.system(cls)
    os.system(cls)  # make sure the entire terminal screen is flushed
...
def deleteRequest(database, index, displayedRequests):
...
def viewOwnRequests(database, member):
    ...
    while len(result) != 0:
        ...
        if len(result) != 0:
            ...
            if fetch.upper() == 'EXIT': #IF the user wants to exit
                ...
                deleteRequest(database, int(fetch) - 1, displayedRequests)
...
def viewOtherRequests(database, member, dbcursor):
        ...
        while len(result) != 0:
            ...
            if len(result) != 0:
                ...
                if fetch.upper() == 'EXIT':  # IF the user wants to exit
                    ...
                    # valid index. delete the request
                    messageRequestOwner(database, int(fetch) - 1, displayedRequests, member)
                    return  # return to main request screen
...
def searchAndDeleteRequest(database, dbcursor,member):
    while(1):
        os.system(cls)
        ...
        if fetch.upper() == 'EXIT': #IF the user wants to exit
            ...
            viewOwnRequests(database, member)


        elif fetch == '2': #valid input
            viewOtherRequests(database, member, dbcursor)
